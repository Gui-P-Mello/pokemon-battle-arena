...
func _apply_sliding_steering(accel: Vector3, delta: float) -> void:
	...
	if calculate_velocities:
		linear_velocity = GSAIUtils.to_vector3(_body.velocity)
...
func _apply_collide_steering(accel: Vector3, delta: float) -> void:
	...
	var collision := _body.move_and_collide(GSAIUtils.to_vector2(velocity) * delta)
	if collision:
		velocity = GSAIUtils.to_vector3(collision.get_remainder())
	...
	if calculate_velocities:
		if _applied_steering:
			...
			angular_velocity = clamp(
				current_orientation - _last_orientation, -angular_speed_max, angular_speed_max
			)
